...
- Topic: Shared question requirements text
  - Verdict: DECISION
  - Rationale: Initial question generation and split generation both need the same rules: binary yes/no, atomic, answer-independent, one concrete requirement, answerable from a future model answer, and positively oriented so `yes` always means the answer satisfied the requirement. Exclusions use positive compliance wording such as "Does the answer avoid ...?"; questions asking whether the answer is wrong, omits content, or includes prohibited content are invalid because deterministic scoring counts `yes`. A shared prompt fragment in `internal/llm/prompts.go` prevents drift.
- Topic: Final scoring
  - Verdict: DECISION
...
- REQ-005 (func): A shared question requirements prompt fragment is used by both dimension question generation and split question generation, and every generated question is score-positive so `yes` always means the evaluated answer satisfies the requirement. Acceptance: tests confirm both prompt builders include the same exported fragment value, require positive orientation, phrase exclusions as compliance checks, forbid questions where `yes` means the answer is wrong, omits required content, or includes prohibited content, and treat independently omittable actor, action, metric/object, ordering, exact-value, and time/duration details as separate checks.
- REQ-006 (func): Weight assignment returns one diagnostic weight object for every candidate question ID, marks semantic duplicates and reverse-polarity questions with `weight=0`, and assigns `2..4` only when the candidate contains that many independently judgeable obligations. Acceptance: missing, duplicate, unknown, blank-rationale, or out-of-range rows fail validation; prompt tests require duplicate removal, score-positive orientation, and compositional split semantics based on details that can be independently present or absent, reject treating a partially satisfiable multi-detail instruction as atomic, and contain no instruction to increase weight merely because an atomic requirement is important.
