...
- Topic: Self-contained CI and real LLM coverage
  - Verdict: DECISION
  - Rationale: CI has two independent gates that reuse the same Compose definition and curl runner. The deterministic gate runs on every change with a schema-conformant OpenAI-compatible fixture and proves product contracts, not model quality. The required live gate runs on trusted `master` and release events with credentials and proves prompt/model quality against a real LLM API. A deterministic success never substitutes for a live failure. When the separately developed caching LLM API is ready, CI changes only `BIN_EVAL_LLM_BASE_URL`; cache hit, miss, and upstream policy remain that service's responsibility, and bin-eval gains no cache-specific branch or adapter.
...
- Dependencies: Go toolchain, Temporal Go SDK, Postgres, Garage, Docker Compose, local LiteLLM Responses API at `http://127.0.0.1:4000/v1/responses`, a CI-reachable real LLM endpoint, an OpenAI-compatible deterministic CI fixture, future caching LLM API service, canonical Make commands, and smoke fixtures under `fixtures/smoke/cases/`.
...
- `P06.S15 Implement deterministic and live full-stack CI jobs`
  - Action: Add hosted CI that runs canonical static/unit/race/integration gates and reuses one Compose definition and curl e2e runner. The deterministic job runs for every change with `BIN_EVAL_LLM_BASE_URL` set to the fixture. The separately required trusted `master` and release job sets the same variable to the real LLM API, or later to the caching API, and fails independently on credentials, reachability, or quality. Upload commit-addressed reports and artifacts.
  - Requirement link: REQ-025, REQ-050.
